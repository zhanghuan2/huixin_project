...
class ServiceGuarantee{
  ...
  init($){
    ...
    if(window.location.host == ctaxccgpHref.split("//")[1]){
      $('.js-login').addClass('for-tax')
      $('.js-login').removeClass('hide')
      self.$btnShopLocated.css('display', 'inline-block')
      $('.js-advantage').hide()
    }else{
      $('.js-login').removeClass('hide')
    }
  ...
  bindEvent(){
    ...
    self.$btnShopLocated.click(function(){
      window.location.href = self.supplierEnv + '/supplier/register-state-tax'
    })
  }
