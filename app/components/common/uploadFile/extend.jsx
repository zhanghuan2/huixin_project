...
class UploadFile {
  ...
  constructor(busiCode,getKeyAPI,downloadUrl,selector){
    if (selector==null) {
      selector="";
    }
    /* 上传成功的文件自定义对象数组 */
    ...
    for(var i=0;i<oldFileDiv.length;i++){
      ...
      var fileSize = $(oldFileDiv[i]).find('span.data-size').text();
      try{$(oldFileDiv[i]).find('span.fileSize').text(formFileSize(fileSize));}catch(e){}
    }
    ...
    $container.find("input[name='files']").bind('change', function(){
      ...
      for(var i=0;i<this.files.length;i++){
        ...
        dcInstance.upload(busiCode,getKeyAPI, [file], {
          error: function (data) {
            /* 显示[上传失败]*/
