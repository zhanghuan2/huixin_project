...
var defaultChecks = [{
  ...
  },
  {
    ...
    check: function(self, object) {
      ...
      return (/^(([0\+]\d{2,3}-)?(0\d{2,3})-)(\d{7,8})(-(\d{3,}))?$/).test(value) || (/^(13|14|15|17|18)\d{9}$/i).test(value) ? true : self.errorText
    }
  },
  {
    type: "Num",
    errorText: "无效数字",
    check: function(self, object) {
      if ((/^\d+(?=\.{0,1}\d+$|$)/).test($(object).val())) {
        $(object).val(parseFloat($(object).val()).toFixed(2));
      }
      return (/^\d+(?=\.{0,1}\d+$|$)/).test($(object).val()) ? true : self.errorText;
    }
  },
  {
    type: "Num2",
  ...
  },
  {
    ...
    errorText: "非正整数",
    check: function(self, object) {
      return (/^[1-9]*[1-9][0-9]*$/).test($(object).val()) ? true : self.errorText;
    }
  },

  {
    ...
    errorText: "非非负整数",
    check: function(self, object) {
      return (/^(0|[1-9]\d*)$/).test($(object).val()) ? true : self.errorText;
    }
  },
  ...
  },
  {
    ...
    errorText: "必输项。",
    check: function(self, object) {
      console.log($(object).attr("data-size"));
...
class formChecker {
  constructor(config) {
    ...
    if (undefined !== _config.extendChecks) {
      $.merge(defaultChecks, _config.extendChecks); // merge array
    }
    ...
    /* 可以作为配置 */
    var $all, $targets, $required, $selectRequired, $checkbox, $length, $compare, $title, $uploadFile_required;
    ...
    $title = $(_config.container).find('input[checker-title]');
    ...
    $title.bind('blur', function() {
      var title = $(this).attr('checker-title');
      var $compares = $("input[checker-title='" + title + "']");
      var base = $compares[0];

      $title.map(function() {
        $(this).removeClass('compare-invalid').addClass('valid');
        /* 如果不是第一个 && 如果值不相等  */
        if ($(this).val().length !== 0 && (title === $(this).val())) {
          $(this).siblings('span[name="check-result"]').remove();
          $(this).addClass('compare-invalid').removeClass('valid');
          that.tips(this, '<span name="check-result" class="text-danger pl">请按照标题模版输入您的标题</span>');
        } else if ($(this).val().length !== 0 && (title !== $(this).val())) {
          /*   如果值相等  */
          $(this).siblings('span[name="check-result"]').remove();
        } else if ($(this).val().length === 0) {
          $(this).removeClass('compare-invalid').removeClass('valid');
    ...
    if (true === this._config.precheck) {
      if (!$targets.size() && !$required.size() && !$length.size() && !$checkbox.size()) {
  ...
  doCheck() {
    ...
    if (!_config.$targets) {
      _config.$targets = $(_config.container).find('input[required],input[checker-type],input[min-length],input[max-length],input[checker-pattern],input[checker-title],' +
    ...
    $targets.map(function() {
      ...
      if ($(this).hasClass('required-invalid') || $(this).hasClass('length-invalid') || 　$(this).hasClass('tool-invalid') || $(this).hasClass('compare-invalid')) {
        $(_config.ctrlTarget).attr('disabled', true);
        // console.log('false1 by', $(this)[0], $(this)[0].id, $(this)[0].name);
        /* 如果需要图标提示 */
        that.icon(this, 'error');
        that.colorBorder(this, 'error');
      } else if ($(this).hasClass('valid')||$(this).hasClass('alid')) {
        /* 如果需要图标提示 */
        ...
        $(_config.ctrlTarget).attr('disabled', true);
        // console.log('false2 by', $(this)[0], $(this)[0].id, $(this)[0].name);
      }
    });
  }
