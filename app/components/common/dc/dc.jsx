...
class dc {
    ...
    download(url, fileId) {
        $.ajax({
            ...
            contentType: 'application/json;charset:utf-8',
            success: function (data) {
                //modify by chenkaixia
                downloadFile(data.downloadUrl||data.result);
            },
            error: function () {
    ...
    /* 上传文件 */
    upload(busiCode,url, files, callback) {
        if (files.length <= 0) {
            return;
        }
        ...
        var vm = this;
        var params={};
        params.fileNum=files.length;
        params.busiCode=busiCode;
        params.key="dc-write";
        $.ajax({
            type: 'GET',
            url: url+"?fileNum="+files.length+"&busiCode="+busiCode+"&key=dc-write",
            dataType: 'json',
            contentType: 'application/json;charset:utf-8',
            success: function (data) {
                var key = data;
