(function($) {
   ...
   */
  let UploadFile = function(element, opts) {
    let _this = this;
  ...
  UploadFile.prototype = {
    utils: {
      ...
      rowTemplate: function(file, showOnly) {
        return `<div class="fileRow" data-file-id="${file.fileId}"><div class="fileMsg"><i class="icon-zcy icon-fujian"></i><span class="fileName">${file.name}</span>${showOnly? '' : '<a class="deleteFile"><i class="icon-zcy icon-guanbi"></i></a>'}</div></div>`;
      }
    },
    ...
    addFileRow: function(file) {
      let _this = this;
      let fileRow = _this.utils.rowTemplate(file, _this.options.showOnly);
      _this.$wrapper.append(fileRow);
    ...
    initDownloadProxy: function() {
      ...
      _this.$element.on('click', '.fileName', function(event) {
        ...
        AliyunUpload.download(_this.options.downUrl, fileId, _this.options.bizCode);
      });
    },
    ...
    init: function(opts) {
      ...
      let notice = `<span class="check-group-tip"><span class="check-group-text">文件大小不能过${_this.utils.formatFileSize(_this.options.maxSize)}</span></span>`
      let button = $('<button type="button" class="btn btn-ghost btn-icon"><span class="icon-zcy icon-shangchuan"></span>上传</button>', { 'class': _this.classes.UploadFileBtn });
      let wrapper = $('<div class="uploadFile"/>').append(input, button, notice);
      let outerWrapper = _this.$element.append(wrapper);
      _this.$wrapper = wrapper;
      ...
      input.off('change').on('change', function(event) {
        ...
        if(window.FileReader) {
          fileOrInput = _this.filterFiles(this.files, function(file) {
            /* 重复文件不操作 */
            let index = _this.findFileByName(file.name);
            return index < 0;
          });
        } else {
          fileOrInput = this;
  ...
  $.fn[pluginName] = function(args) {
    return this.each(function() {
      ...
      if(data && !data.disableOnMobile) {
        ...
        $.data(this, pluginName, new UploadFile(this, args));
      }
    });
  };
